...
/*
 * compare two different possibilities to calculate the number of integer
 * compositions
 */
...
void test0() {
  ...
  for (uint k = 1; k < 4; ++k) {
    for (auto n : N) {
      ...
      std::cout << k << ' ' << std::setw(3) << n << ' ' << std::setw(12)
                << lNoDecomp1 << ' ' << std::setw(12) << lNoDecomp2
                << std::endl;
    }
  }
  std::cout << std::endl;
 ...
 *  they use different implementations of calculating the number of integer
 * compositions these estimators are also implemented in SimpleProfileGbh
 */

double f_est_sum_eq_1(const double p, const uint n, const uint m,
                      const uint_vt& F) {
  double lRes = 0;
...
double f_est_sum_eq_2(const double p, const uint n, const uint m,
                      const uint_vt& F) {
  assert(n <= m);
  double lRes = 0;
  for (uint k = 1; k < F.size(); ++k) {
...
double f_est_sum_eq_3(const double p, const uint n, const uint m,
                      const uint_vt& F) {
  double lRes = 0;
  for (uint k = 1; k < F.size(); ++k) {
...
void test1(const TpchEst& aTpchEst) {
  ...
  for (uint b = 5; b <= 50; b += 5) {
    ...
    std::cout << std::setw(2) << b << "  " << std::setw(10) << lTrue << "  "
              << std::setw(10) << lEst1 << "  " << std::setw(10)
              << q::qerror<double>(lEst1, lTrue);
    if (lEst2 != lEst1) {
  ...
  std::cout << "having sum(l_orderkey) <=  50: " << ((uint64_t)std::round(lRes))
            << std::endl;
  ...
  std::cout << "having sum(l_orderkey) >  300: " << ((uint64_t)std::round(lRes))
            << std::endl;
}
...
double run_loop_sum_eq_a(const TpchEst& aTpchEst, const uint aBegin,
                         const uint aEnd, const uint aStep,
  ...
  for (uint n = aBegin; n < aEnd; n += aStep) {
    ...
    const double lQEr =
        q::qerror(std::max<double>(1, lEst), std::max<double>(1, lTru));
    lQErrMax = std::max<double>(lQEr, lQErrMax);
    if (aTrace && (aQErrLim < lQEr)) {
      std::cout << std::setw(3) << n << "  " << std::setw(8)
                << ((uint64_t)std::round(lEst)) << "  " << std::setw(8)
                << ((uint64_t)lTru) << "  " << std::setw(8)
                << mt::roundXXt<double>(lQEr) << std::endl;
    }
  }
  if (aTrace) {
...
void test2(const bool aProduceProbTable) {
  ...
  lVar.fin();
  std::cout << "l_quantity in [1,50]:" << "  mean = " << lVar.mean()
            << ", var = " << lVar.variance()
            << ", stddev = " << lVar.standardDeviation() << std::endl
            << std::endl;
  ...
  std::cout << "max-qerr in loop b in 1-199: "
            << run_loop_sum_eq_a(lTpchEst, 1, 200, 1, 7.77, false) << std::endl;
  ...
  std::cout << "cases where q-error > " << lMaxQErrAllowed
            << " for b in [1,350]:" << std::endl;
  ...
  std::cout << "having sum(l_orderkey) <= 50: " << "  " << std::setw(8)
            << ((uint64_t)std::round(lEst)) << "  " << std::setw(8)
            << ((uint64_t)std::round(lTru)) << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst, lTru)) << std::endl;

  lEst = lTpchEst.estimate_sum_between(301, 350);
  lTru = lTpchEst.get_true_card_sum_between(301, 350);
  std::cout << "having sum(l_orderkey) > 300: " << "  " << std::setw(8)
            << ((uint64_t)std::round(lEst)) << "  " << std::setw(8)
            << ((uint64_t)std::round(lTru)) << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst, lTru)) << std::endl;

  lEst = lTpchEst.estimate_sum_between(1, 19);
  std::cout << "est[sum(qty) < 20, ----] = " << std::setw(8) << (uint64_t)lEst
            << std::endl;
  lEst = lTpchEst.estimate_sum_between_and_cnt_between(1, 19, 1, 4);
  std::cout << "est[sum(qty) < 20, 1..4] = " << std::setw(8) << (uint64_t)lEst
            << std::endl;
  lEst = lTpchEst.estimate_sum_between_and_cnt_between(1, 19, 4, 7);
  std::cout << "est[sum(qty) < 20, 4..7] = " << std::setw(8) << (uint64_t)lEst
            << std::endl;

  lEst = lTpchEst.estimate_sum_between(1, 49);
  std::cout << "est[nosel  , sum(qty) < 50] = " << std::setw(8)
            << (uint64_t)lEst << std::endl;
  lEst = lTpchEst.estimate_sum_between_sel(1, 49, 0.5);
  std::cout << "est[sel=0.5, sum(qty) < 50] = " << std::setw(8)
            << (uint64_t)lEst << std::endl;
...
void test3() {
  ...
  lVar.fin();
  std::cout << "[1,50]: mean = " << lVar.mean() << ", var = " << lVar.variance()
            << ", stddef = " << lVar.standardDeviation() << std::endl;
  std::cout << std::endl;
}
...
void mk_table_count_sqlserver(const EstHavingSqlServer& aSqlServerEst,
  ...
  const int w = 12;
  std::cout << " c" << " & " << std::setw(w)
            << "true card"
            // << " & " << std::setw(w) << "selectivity"
            << " & " << std::setw(w) << "estimate" << " & " << std::setw(w)
            << "q-error" << std::endl;
  for (uint k = 1; k <= 10; ++k) {
    ...
    std::cout << std::setw(2) << k << " & " << std::setw(w)
              << lTrue
              // << " & " << std::setw(w) << lSeli
              << " & " << std::setw(w) << lEst1 << " & " << std::setw(w)
              << mt::roundXXt(lQErr) << "  \\\\" << std::endl;
  }
}
...
// check having count(*) ...
void test4(const SimpleProfileGbH& aSimpEst, const TpchEst& aTpchEst,
           const EstHavingSqlServer& aSqlServerEst,
           const EstHavingFentSn& aEstFentSn, const EstHavingBeta aEstBeta,
           const Cb& aCb) {
  ...
  std::cout << "  c true-card"
            ...
            << std::endl;
  */
  std::cout << "  c" << "  SqlServer " << "  Fent      " << "  Beta      "
            << "  SimProfile" << std::endl;
  ...
  for (uint lCnt = 0; lCnt <= 8; ++lCnt) {
    ...
    std::cout << std::setw(3)
              ...
              << "  " << std::setw(7)
              << mt::roundXXXt<double>(lQEr[0])  // SqlServer

              // << "  " << std::setw(9) << ((uint64_t) std::round(lEst[1]))
              << "  " << std::setw(7)
              << mt::roundXXXt<double>(lQEr[1])  // Fent

              // << "  " << std::setw(9) << ((uint64_t) std::round(lEst[2])) //
              // Beta-Distribution
              << "  " << std::setw(7)
              << mt::roundXXXt<double>(lQEr[2])

              // << "  " << std::setw(9) << ((uint64_t) std::round(lEst[3]))
              << "  " << std::setw(7)
              << mt::roundXXXt<double>(lQEr[3])  // SimpleProfile

              << std::endl;
  }
  std::cout << std::endl;
  testcase_eq_vt lCases = {{2, 1, 413450},  {3, 1, 517230}, {4, 1, 542115},
                           {10, 1, 399855}, {2, 4, 155767}, {3, 4, 56765},
                           {4, 4, 23179},   {10, 4, 933}};
  ...
  for (uint i = 0; i < lCases.size(); ++i) {
    ...
    lQEr[0] = q::qerror(lEst[0], lTru);
    std::cout << std::setw(3) << lCases[i].val() << "  " << std::setw(3)
              << lCases[i].mod() << "  " << std::setw(8) << ((uint64_t)lTru)
              << "  " << std::setw(8) << ((uint64_t)std::round(lEst[0])) << "  "
              << std::setw(8) << mt::roundXXXt<double>(lQEr[0]) << std::endl;
  }
  std::cout << std::endl;
}

// check having sum(l_quantity)...
double_vt run_loop_sum_eq_b(const SimpleProfileGbH& aSimpleEst,
                            const TpchEst& aTpchEst,
                            const EstHavingFentSn& aEstFentSn,
                            const EstHavingBeta& aEstBeta, const uint aBegin,
                            const uint aEnd, const uint aStep,
  ...
  if (aTrace) {
    std::cout << "estimates for equality queries with q-error > " << aQErrLim
              << ':' << std::endl;
    std::cout << "  b true-card" << "   TpchEst   q-error"
              << "  SimplEst   q-error" << " FentSnEst   q-error"
              << "      Beta   q-error" << std::endl;
  }
  for (uint n = aBegin; n < aEnd; n += aStep) {
    const double lTru = aTpchEst.get_true_card_sum_eq(n);
    ...
    lQEr[0] =
        q::qerror(std::max<double>(1, lEst[0]), std::max<double>(1, lTru));
    lQEr[1] =
        q::qerror(std::max<double>(1, lEst[1]), std::max<double>(1, lTru));
    lQEr[2] =
        q::qerror(std::max<double>(1, lEst[2]), std::max<double>(1, lTru));
    lQEr[3] =
        q::qerror(std::max<double>(1, lEst[3]), std::max<double>(1, lTru));
    ...
    if (aTrace && (aQErrLim < lQEr[1])) {
      std::cout << std::setw(3) << n << "  " << std::setw(8) << ((uint64_t)lTru)
                ...
                << std::setw(8) << mt::roundXXt<double>(lQEr[3]) << std::endl;
    }
  }
  if (aTrace) {
    std::cout << "max-qerror: " << mt::roundXXt(lQErrMax[0]) << ' '
              << mt::roundXXt(lQErrMax[1]) << ' ' << mt::roundXXt(lQErrMax[2])
              << ' ' << mt::roundXXt(lQErrMax[3]) << std::endl
              << std::endl;
  }
  return lQErrMax;
}
...
// check having sum(l_quantity)...
double_vt run_loop_sum_eq_c(const SimpleProfileGbH& aSimpleEst,
                            const TpchEst& aTpchEst,
                            const EstHavingFentSn& aEstFentSn,
                            const EstHavingBeta& aEstBeta, const uint aBegin,
                            const uint aEnd, const uint aStep,
  ...
  if (aTrace) {
    std::cout << "estimates for equality queries with q-error > " << aQErrLim
              << ':' << std::endl;
    std::cout << "  b true-card" << "      Fent   q-error"
              << "      Beta   q-error" << "     SP(1)   q-error"
              << "     SP(1)   q-error" << "     SP(2)   q-error"
              << "     SP(3)   q-error" << "        IC   q-error" << std::endl;
  }
  for (uint n = aBegin; n < aEnd; n += aStep) {
    const double lTru = aTpchEst.get_true_card_sum_eq(n);
    ...
    for (uint i = 0; i < lNoEst; ++i) {
      lQEr[i] =
          q::qerror(std::max<double>(1, lEst[i]), std::max<double>(1, lTru));
...
std::ostream& range_t::print(std::ostream& os, const int aWidth) const {
  os << std::setw(0) << '[' << std::setw(aWidth) << min() << ','
     << std::setw(aWidth) << max() << ']';
  return os;
}

std::ostream& operator<<(std::ostream& os, const range_t& x) {
...
// check having sum() ...
void test5(const SimpleProfileGbH& aSimpEst, const TpchEst& aTpchEst,
           const EstHavingFentSn& aEstFentSn, const EstHavingBeta aEstBeta,
           const Cb& aCb) {
  ...
  /*
    ...
    lQErr1 = run_loop_sum_eq_b(aSimpEst, aTpchEst, aEstFentSn, aEstBeta, 1, 200,
    1, 7.77, lTraceLoopB); if(lTraceLoopB) { std::cout << "END TRACE" <<
    std::endl;
    }
  */
  ...
  std::cout << "                               " << ' ' << std::setw(8)
            << "TpchEst1" << ' ' << std::setw(8) << "TpchEst2" << ' '
            << std::setw(8) << "SimpEst" << ' ' << std::setw(8) << "Fent" << ' '
            << std::setw(8) << "Beta" << std::endl;

  for (const auto& lRange : lRangesB) {
    lQErr0 =
        run_loop_sum_eq_a(aTpchEst, lRange.min(), lRange.max(), 1, 7.77, false);
    lQErr1 = run_loop_sum_eq_b(aSimpEst, aTpchEst, aEstFentSn, aEstBeta,
                               lRange.min(), lRange.max(), 1, 7.77, false);
    std::cout << "  max-qerr for b in " << std::setw(3) << lRange << ": " << ' '
              << std::setw(8) << mt::roundXXt(lQErr0) << ' ' << std::setw(8)
              << mt::roundXXt(lQErr1[0]) << ' ' << std::setw(8)
              << mt::roundXXt(lQErr1[1]) << ' ' << std::setw(8)
              << mt::roundXXt(lQErr1[2]) << ' ' << std::setw(8)
              << mt::roundXXt(lQErr1[3]) << std::endl;
  }
  ...
  for (uint i = 0; i < lRangesB.size(); ++i) {
    ...
    lQErr1 = run_loop_sum_eq_c(aSimpEst, aTpchEst, aEstFentSn, aEstBeta,
                               lRange.min(), lRange.max(), 1, 7.77, false);
    lTable[i] = lQErr1;
    std::cout << "  max-qerr for b in " << std::setw(3) << lRange << ":";
  ...
  const string_vt lNames = {"Fent",  "Beta",  "SP", "SP(1)",
                            "SP(2)", "SP(3)", "IC"};
  ...
  const double lMaxQErrAllowed = 1.5;
  std::cout << "  all cases where q-error > " << lMaxQErrAllowed
            << " for b in [1,350]:" << std::endl;
  run_loop_sum_eq_b(aSimpEst, aTpchEst, aEstFentSn, aEstBeta, 1, 351, 1,
                    lMaxQErrAllowed, true);
  ...
  std::cout << "------------------------" << std::endl;
  std::cout << std::string(30, ' ') << "  " << "true card" << "  " << "tpch-est"
            << "  " << "q-err" << "  " << "simp-est" << "  " << "q-err" << "  "
            << "Fent(SN)" << "  " << "q-err" << "  " << "    Beta" << "  "
            << "q-err" << std::endl;
  ...
  std::cout << "  sum(l_quantity) <=  50      :" << "  " << std::setw(8)
            << ((uint64_t)std::round(lTrue)) << "  " << std::setw(8)
            << ((uint64_t)std::round(lEst[0])) << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[0], lTrue)) << "  " << std::setw(8)
            << ((uint64_t)std::round(lEst[1])) << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[1], lTrue)) << "  " << std::setw(8)
            << ((uint64_t)std::round(lEst[2])) << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[2], lTrue)) << "  " << std::setw(8)
            << ((uint64_t)std::round(lEst[3])) << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[3], lTrue)) << std::endl;
  ...
  std::cout << "  sum(l_quantity) >  300      :" << "  " << std::setw(8)
            << ((uint64_t)std::round(lTrue)) << "  " << std::setw(8)
            << ((uint64_t)std::round(lEst[0])) << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[0], lTrue)) << "  " << std::setw(8)
            << ((uint64_t)std::round(lEst[1])) << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[1], lTrue)) << "  " << std::setw(8)
            << ((uint64_t)std::round(lEst[2])) << "  " << std::setw(5)
            << mt::roundXt(q::qerror(lEst[2], lTrue)) << "  " << std::setw(8)
            << ((uint64_t)std::round(lEst[3])) << "  " << std::setw(5)
            << mt::roundXt(q::qerror(lEst[3], lTrue)) << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "  sum(l_quantity) <   20, ----:" << "  " << std::setw(8)
            << ((uint64_t)std::round(lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[0] << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[0], lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[1] << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[1], lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[2] << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[2], lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[3] << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[3], lTrue)) << std::endl;
  ...
  // lEst[2] = aEstFentSn.estimate_sum_between_and_cnt_between(1, 19, 1, 4); //
  // does not exist
  std::cout << "  sum(l_quantity) <   20, 1..4:" << "  " << std::setw(8)
            << ((uint64_t)std::round(lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[0] << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[0], lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[1] << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[1], lTrue)) << std::endl;
  ...
  // lEst[2] = aEstFentSn.estimate_sum_between_and_cnt_between(1, 19, 4, 7); //
  // does not exist
  std::cout << "  sum(l_quantity) <   20, 4..7:" << "  " << std::setw(8)
            << ((uint64_t)std::round(lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[0] << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[0], lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[1] << "  " << std::setw(5)
  ...
  // lEst[2] = aEstFentSn.estimate_sum_between_sel(1, 49, 0.5); // does not
  // exist
  std::cout << "  sum(l_quantity) <   50,  0.5:" << "  " << std::setw(8)
            << ((uint64_t)std::round(lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[0] << "  " << std::setw(5)
            << mt::roundXXXt(q::qerror(lEst[0], lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[1] << "  " << std::setw(5)
  ...
  // lEst[2] = aEstFentSn.estimate_sum_between_sel(51, 350, 0.5); // does not
  // exist
  std::cout << "  sum(l_quantity) >   50,  0.5:" << "  " << std::setw(8)
            << ((uint64_t)std::round(lTrue)) << "  " << std::setw(8)
            << (uint64_t)lEst[0] << "  " << std::setw(5)
...
void test6(const SimpleProfileGbH& aSimpEst, const TpchEst& aTpchEst,
           const Cb& aCb) {
  ...
  for (uint lLb = 0; lLb <= 50; lLb += 5) {
    ...
    std::cout << std::setw(2) << lLb << ',' << std::setw(2) << lUb << "  "
              << std::setw(8) << ((uint64_t)lTru) << "  " << std::setw(8)
              << ((uint64_t)std::round(lEst)) << "  " << std::setw(8)
              << mt::roundXXt<double>(lQEr) << std::endl;
  }
}

void test7a(const SimpleProfileGbH& aSimpEst, const TpchEst& aTpchEst,
            const Cb& aCb) {
  std::cout << "=== test7a ===" << std::endl;
  testcase_eq_vt lCases{
      {0, 5, 83067},  {0, 10, 55725}, {0, 15, 37057}, {0, 20, 24829},
      {0, 25, 17031}, {0, 30, 12141}, {0, 35, 8981},  {0, 40, 6752},
      {0, 45, 5315},  {0, 50, 4441},
  };
  ...
  for (uint i = 0; i < lCases.size(); ++i) {
    ...
    lQEr = q::qerror(lEst, lTru);
    std::cout << std::setw(3) << lCases[i].val() << "  " << std::setw(8)
              << ((uint64_t)lTru) << "  " << std::setw(8)
              << ((uint64_t)std::round(lEst)) << "  " << std::setw(8)
              << mt::roundXXXt<double>(lQEr) << std::endl;
  }
}

void test7b(const SimpleProfileGbH& aSimpEst, const TpchEst& aTpchEst,
            const Cb& aCb) {
  std::cout << "=== test7b ===" << std::endl;
  testcase_eq_vt lCases{{0, 1, 4372},   {0, 4, 4978},   {0, 7, 5641},
                        {0, 10, 6663},  {0, 13, 7567},  {0, 16, 9066},
                        {0, 19, 10814}, {0, 22, 13046}, {0, 25, 16006},
                        {0, 28, 19909}, {0, 31, 24782}, {0, 34, 31464},
                        {0, 37, 40102}, {0, 40, 51202}, {0, 43, 65035},
                        {0, 46, 83601}, {0, 49, 106214}};
...
double get_estimate(const SimpleProfileGbH& aSp, const queryinstance_t& aInst,
  ...
  if (kPredEq == aQueryTemplate.pred_having_aggr()) {
    ...
    if (kNoAndOr == aQueryTemplate.bool_connector()) {
      if (kNoPred == aQueryTemplate.pred_where()) {
        switch (aQueryTemplate.aggr_having()) {
          case kAggrCnt:
            lRes = aSp.estimate_cnt_eq(aInst.a());
            break;
          case kAggrSum:
            lRes = aSp.estimate_sum_eq(aInst.a());
            break;
          case kAggrAvg:
            std::cout << "NYI: avg(.) = " << std::endl;
            assert(0 == 3);
            break;
          case kAggrMin:
            lRes = aSp.estimate_min_eq(aInst.a());
            break;
          case kAggrMax:
            lRes = aSp.estimate_max_eq(aInst.a());
            break;
        }
      } else {
        switch (aQueryTemplate.aggr_having()) {
          case kAggrCnt:
            lRes = aSp.estimate_cnt_eq_sel(aInst.a(), aInst.sel());
            break;
          case kAggrSum:
            lRes = aSp.estimate_sum_eq_sel(aInst.a(), aInst.sel());
            break;
          case kAggrAvg:
            std::cout << "NYI: avg(.) = " << std::endl;
            assert(0 == 4);
            break;
          case kAggrMin:
            lRes = aSp.estimate_min_eq_sel(aInst.a(), aInst.sel());
            break;
          case kAggrMax:
            lRes = aSp.estimate_max_eq_sel(aInst.a(), aInst.sel());
            break;
        }
      }
    } else if (kAnd == aQueryTemplate.bool_connector()) {
      // ... and count(*) between ...
      if (kNoPred == aQueryTemplate.pred_where()) {
        switch (aQueryTemplate.aggr_having()) {
          case kAggrCnt:
            ...
            break;
          case kAggrSum:
            lRes = aSp.estimate_sum_eq_cnt(aInst.a(), aInst.c(), aInst.d());
            break;
          case kAggrAvg:
            std::cout << "NYI: avg(.) = " << std::endl;
            assert(0 == 5);
            break;
          case kAggrMin:
            lRes = aSp.estimate_min_eq_cnt(aInst.a(), aInst.c(), aInst.d());
            break;
          case kAggrMax:
            lRes = aSp.estimate_max_eq_cnt(aInst.a(), aInst.c(), aInst.d());
            break;
        }
      } else {
        std::cout << "NYI: where p having aggr(.) ... and count(*) between ..."
                  << std::endl;
        assert(0 == 2);
      }
    } else if (kOr == aQueryTemplate.bool_connector()) {
    ...
    if (kNoAndOr == aQueryTemplate.bool_connector()) {
      if (kNoPred == aQueryTemplate.pred_where()) {
        switch (aQueryTemplate.aggr_having()) {
          case kAggrCnt:
            lRes = aSp.estimate_cnt_between(aInst.a(), aInst.b());
            break;
          case kAggrSum:
            lRes = aSp.estimate_sum_between(aInst.a(), aInst.b());
            break;
          case kAggrAvg:
            lRes = aSp.estimate_avg_between(aInst.a(), aInst.b());
            break;
          case kAggrMin:
            lRes = aSp.estimate_min_between(aInst.a(), aInst.b());
            break;
          case kAggrMax:
            lRes = aSp.estimate_max_between(aInst.a(), aInst.b());
            break;
        }
      } else {
        switch (aQueryTemplate.aggr_having()) {
          case kAggrCnt:
            lRes =
                aSp.estimate_cnt_between_sel(aInst.a(), aInst.b(), aInst.sel());
            break;
          case kAggrSum:
            lRes =
                aSp.estimate_sum_between_sel(aInst.a(), aInst.b(), aInst.sel());
            break;
          case kAggrAvg:
            lRes =
                aSp.estimate_avg_between_sel(aInst.a(), aInst.b(), aInst.sel());
            break;
          case kAggrMin:
            lRes =
                aSp.estimate_min_between_sel(aInst.a(), aInst.b(), aInst.sel());
            break;
          case kAggrMax:
            lRes =
                aSp.estimate_max_between_sel(aInst.a(), aInst.b(), aInst.sel());
            break;
        }
      }
    } else if (kAnd == aQueryTemplate.bool_connector()) {
      ...
      if (kNoPred == aQueryTemplate.pred_where()) {
        switch (aQueryTemplate.aggr_having()) {
          case kAggrCnt:
            ...
            break;
          case kAggrSum:
            lRes = aSp.estimate_sum_between_and_cnt_between(
                aInst.a(), aInst.b(), aInst.c(), aInst.d());
            break;
          case kAggrAvg:
            lRes = aSp.estimate_avg_between_and_cnt_between(
                aInst.a(), aInst.b(), aInst.c(), aInst.d());
            break;
          case kAggrMin:
            lRes = aSp.estimate_min_between_and_cnt_between(
                aInst.a(), aInst.b(), aInst.c(), aInst.d());
            break;
          case kAggrMax:
            lRes = aSp.estimate_max_between_and_cnt_between(
                aInst.a(), aInst.b(), aInst.c(), aInst.d());
            break;
        }
      } else {
...
bool read_data(double_vt& aCardTru, queryinstance_vt& aQueries,
...
double run_estimates(const SimpleProfileGbH& aSimpEst,
                     const double_vt& aCardTru,
                     const queryinstance_vt& aQueries,
                     const querytemplate_t& aTemp, const Cb& aCb) {
  ...
  if (aCb.trace() || aCb.printBad()) {
    ...
    std::cout << "  " << std::setw(8) << "true" << "  " << std::setw(8) << "est"
              << "  " << std::setw(8) << "qerr" << std::endl;
  }
  for (uint i = 0; i < aCardTru.size(); ++i) {
    lTru = aCardTru[i];
    lEst = get_estimate(aSimpEst, aQueries[i], aTemp);
    lQEr =
        q::qerror<double>(std::max<double>(1, lEst), std::max<double>(1, lTru));
    if (lQEr > lQErrMax) {
      lQErrMax = lQEr;
    }
    if (((aCb.theta() < lTru) || (aCb.theta() < lEst)) &&
    ...
    if (aCb.trace() || (aCb.printBad() && (aCb.lim_qerr() < lQEr))) {
      ...
      std::cout << "  " << std::setw(8) << ((uint)lTru) << "  " << std::setw(8)
                << ((uint)std::round(lEst)) << "  " << std::setw(8) << lQEr
                << std::endl;
    }
  }
  return lQErrMax;
...
void test8(const SimpleProfileGbH& aSimpEst, const TpchEst& aTpchEst,
           const Cb& aCb) {
  if (aCb.trace()) {
  ...
  const uint lCaseMax =
      (0 == aCb.caseNo() ? querytemplate_t::no_templates_to_gen()
                         : aCb.caseNo());
  std::cout << std::string(28, ' ') << "#query  maxqerr     qxerr (theta)"
            << std::endl;
  for (uint i = lCaseMin; i <= lCaseMax; ++i) {
    lTemp.set(i);
    std::cout << "case " << std::setw(2) << i << "  " << std::left
              << std::setw(17) << lTemp.to_string() << std::right << ": ";
    if (read_data(lCardTru, lQueries, lTemp)) {
      ...
      const double lQErrMax =
          run_estimates(aSimpEst, lCardTru, lQueries, lTemp, aCb);
      std::cout << std::fixed;
      std::cout.precision(2);
      std::cout << ' ' << std::setw(8) << mt::roundXXt(lQErrMax) << ' '
                << std::setw(8) << mt::roundXXt(aCb._theta_qerr_max) << " ("
                << (uint)mt::roundXXt(aCb.theta()) << ')' << std::endl;
    } else {
...
double get_true_card(const TpchEst& aTpchEst, queryinstance_t aInst,
                     const querytemplate_t aQueryTemplate) {
  uint lRes = 0;
  if (kPredEq == aQueryTemplate.pred_having_aggr()) {
    // having aggr(.) = ...
    if (kNoAndOr == aQueryTemplate.bool_connector()) {
      if (kNoPred == aQueryTemplate.pred_where()) {
        switch (aQueryTemplate.aggr_having()) {
          case kAggrCnt:
            lRes = aTpchEst.get_true_card_cnt_eq(aInst.a());
            break;
          case kAggrSum:
            lRes = aTpchEst.get_true_card_sum_eq(aInst.a());
            break;
          case kAggrAvg:
            assert(0 == 3);
            break;
      ...
      if (kNoPred == aQueryTemplate.pred_where()) {
        switch (aQueryTemplate.aggr_having()) {
          case kAggrCnt:
            ...
            break;
          case kAggrSum:
            lRes = aTpchEst.get_true_card_sum_eq_and_cnt_between(
                aInst.a(), aInst.c(), aInst.d());
            break;
          case kAggrAvg:
            assert(0 == 6);
            break;
    ...
    if (kNoAndOr == aQueryTemplate.bool_connector()) {
      if (kNoPred == aQueryTemplate.pred_where()) {
        switch (aQueryTemplate.aggr_having()) {
          case kAggrCnt:
            lRes = aTpchEst.get_true_card_cnt_between(aInst.a(), aInst.b());
            break;
          case kAggrSum:
            lRes = aTpchEst.get_true_card_sum_between(aInst.a(), aInst.b());
            break;
          case kAggrAvg:
            lRes = aTpchEst.get_true_card_avg_between(aInst.a(), aInst.b());
            break;
          case kAggrMin:
            assert(0 == 11);
            break;
      ...
      if (kNoPred == aQueryTemplate.pred_where()) {
        switch (aQueryTemplate.aggr_having()) {
          case kAggrCnt:
            ...
            break;
          case kAggrSum:
            lRes = aTpchEst.get_true_card_sum_between_and_cnt_between(
                aInst.a(), aInst.b(), aInst.c(), aInst.d());
            break;
          case kAggrAvg:
            lRes = aTpchEst.get_true_card_avg_between_cnt(aInst.a(), aInst.b(),
                                                          aInst.c(), aInst.d());
            break;
          case kAggrMin:
            assert(0 == 14);
...
void eval_query(const SimpleProfileGbH& aSimpEst, const TpchEst& aTpchEst,
                queryinstance_t& aQueryInstance,
                const querytemplate_t& aQueryTemplate, const Cb& aCb) {
  ...
  if (aCb.trace() || (aCb.printBad() && (aCb.lim_qerr() < lQEr))) {
    ...
    std::cout << "  " << std::setw(8) << lTru << "  " << std::setw(8)
              << ((uint64_t)std::round(lEst)) << "  " << std::setw(8)
              << mt::roundXXt(lQEr) << std::endl;
  }
}

void eval_having_count(const SimpleProfileGbH& aSimpEst,
                       const TpchEst& aTpchEst, queryinstance_t& aQueryInstance,
                       const querytemplate_t& aQueryTemplate, const Cb& aCb) {
  loopspec_t lLoop;
  if (kNoAndOr == aQueryTemplate.bool_connector()) {
  ...
  if (kPredEq == aQueryTemplate.pred_having_cnt()) {
    lLoop.set(1, 7, 1);
  } else if (kPredRg == aQueryTemplate.pred_having_cnt()) {
    lLoop.set(1, 7, 1, 2, 5, 1);
  }

  lLoop.eval_estimates(&eval_query, aQueryInstance.get_c(),
                       aQueryInstance.get_d(), aSimpEst, aTpchEst,
                       aQueryInstance, aQueryTemplate, aCb);
}

void eval_having_aggr(const SimpleProfileGbH& aSimpEst, const TpchEst& aTpchEst,
                      queryinstance_t& aQueryInstance,
                      const querytemplate_t& aQueryTemplate, const Cb& aCb) {
  loopspec_t lLoop;
  if (kAggrCnt == aQueryTemplate.aggr_having()) {
    if (kPredEq == aQueryTemplate.pred_having_aggr()) {
      lLoop.set(1, 7, 1);
    } else if (kPredRg == aQueryTemplate.pred_having_aggr()) {
      lLoop.set(1, 7, 1, 2, 5, 1);
    }
  } else if (kAggrSum == aQueryTemplate.aggr_having()) {
    if (kPredEq == aQueryTemplate.pred_having_aggr()) {
      lLoop.set(1, 300, 1);
    } else if (kPredRg == aQueryTemplate.pred_having_aggr()) {
      lLoop.set(1, 300, 1, 2, 10, 1);
    }
  } else if (kAggrAvg == aQueryTemplate.aggr_having()) {
    if (kPredEq == aQueryTemplate.pred_having_aggr()) {
      assert(0 == 1);
    } else {
      lLoop.set(1, 300, 1, 2, 10, 1);
    }
  } else if ((kAggrMin == aQueryTemplate.aggr_having()) ||
  ...
  lLoop.eval_estimates(&eval_having_count, aQueryInstance.get_a(),
                       aQueryInstance.get_b(), aSimpEst, aTpchEst,
                       aQueryInstance, aQueryTemplate, aCb);
}

void eval_queries(const querytemplate_t& aQueryTemplate,
                  const SimpleProfileGbH& aSimpEst, const TpchEst& aTpchEst,
                  const Cb& aCb) {
  queryinstance_t lQueryInstance;
  eval_having_aggr(aSimpEst, aTpchEst, lQueryInstance, aQueryTemplate, aCb);
}

void test9(const SimpleProfileGbH& aSimpEst, const TpchEst& aTpchEst,
           const Cb& aCb) {
  if (aCb.trace()) {
    std::cout << "=== test9 ===" << std::endl;
  }
  const uint lBegin =
      std::max<uint>(querytemplate_t::no_templates_to_gen() + 1, aCb.caseNo());
  const uint lEnd =
      (0 == aCb.caseNo() ? querytemplate_t::no_templates() : aCb.caseNo());
  ...
  for (uint i = lBegin; i <= lEnd; ++i) {
    ...
    std::cout << "case " << std::setw(2) << i << "  " << std::left
              << std::setw(17) << lQueryTemplate.to_string() << std::right
              << ": ";
    if (aCb.trace() || aCb.printBad()) {
      std::cout << std::endl;
    }
    eval_queries(lQueryTemplate, aSimpEst, aTpchEst, aCb);
    std::cout << ' ' << std::setw(5) << aCb._count << ' ' << std::setw(8)
              << mt::roundXXt(aCb._qerr_max) << ' ' << std::setw(8)
              << mt::roundXXt(aCb._theta_qerr_max) << ' ' << '('
              << (uint)mt::roundXXt(aCb.theta()) << ')' << std::endl;
  }
}
...
int main(const int argc, const char* argv[]) {
  ...
  if (lCb.help()) {
    std::cout
        ...
        << "|  " << argv[0] << "  |" << std::endl
        << "|--" << std::string(strlen(argv[0]), '-') << "---" << std::endl
        << "| produces estimates and q-errors for different estimation tasks."
        << std::endl
        ...
        << "|  " << argv[0] << " pat" << std::endl
        << "| Here, there are 25 test cases available, corresponding to 25 "
           "different"
        << std::endl
        << "| estimation procedures in SimpleProfileGbH." << std::endl
        << "| cases 1-17 require prior evaluation of SQL queries generated via "
           "main_gen."
        << std::endl
        << "| For the remaining cases TpchEst is able to give the correct "
           "result cardinality."
        << std::endl
        << "| If only a single test case is to be run, use -n" << std::endl;
  ...
  // test0(); // checks on number of compositions, deprecated (see
  // infra/integer_compositions.hh/cc)
  if (1 == argc) {
    test1(lTpchEst);  // having sum(B) ...; using Sec. 4.4 Integer B: counting
                      // integer compositions
    std::cout << std::endl;
  }
  // test2(false);  // check estimates of TpchEst, generate probability table
  // z_probtable.dat test3();       // check Variance<> for sums

  const double lMean =
      (lCb.calcMeanVar() ? uni_dist_mean(1, 50) : 25.507967136655);
  const double lVari =
      (lCb.calcMeanVar() ? uni_dist_variance(1, 50) : 208.117016107785);
  const double lStdD = std::sqrt(lVari);
  const double lSkew =
      0;  // skew of uniform distribution on [a,b] is always zero
  ...
  lAttrStatB._dist_param._std_dev =
      lCb.calcMeanVar() ? lStdD : 14.426262537016854;
  lAttrStatB._dist_param._skewness =
      lCb.calcMeanVar() ? lSkew : -0.0010518682122090267;
  lAttrStatA._is_int = true;
  lAttrStatA._is_ui_dense = lCb.isUiDense();

  // instantiate simple profile
  SimpleProfileGbH lSimpleProfile(lCardR, lAttrStatA, lAttrStatB, lAttrStatCnt);

  // get instance of SqlServerEst
  EstHavingSqlServer lSqlServerEst(lCardR, lAttrStatA, lAttrStatB,
                                   lAttrStatCnt);

  // get instance of EstFentSn
  EstHavingFentSn lEstFentSn(lCardR, lAttrStatA, lAttrStatB, lAttrStatCnt);

  // gest instance of EstBeta
  EstHavingBeta lEstBeta(lCardR, lAttrStatA, lAttrStatB, lAttrStatCnt);
  // some tracing

  if (lCb.trace()) {
    std::cout << "# main_having" << " -i  " << lCb.isUiDense() << " -c  "
              << lCb.calcMeanVar() << " -n  " << lCb.caseNo() << " -q  "
              << lCb.lim_qerr() << " -theta " << lCb.theta() << ' '
              << lCb.pat_or_aggr() << std::endl;
  }
  ...
  if ("pat" == lCb.pat_or_aggr()) {
    ...
    if ((0 == lCb.caseNo()) ||
        (querytemplate_t::no_templates_to_gen() < lCb.caseNo())) {
