...
double dist_param_t::moment_3() const {
  const double lMom2 = moment_2();
  return ((skew() * std_dev() * std_dev() * std_dev()) -
          2 * mean() * mean() * mean() + 3 * mean() * lMom2);
}

dist_param_t operator*(const dist_param_t& x, const dist_param_t& y) {
...
bool dist_param_t::get_param_skew_norm_b(double& aXi, double& aEta,
