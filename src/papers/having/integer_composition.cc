...
uint64_t f_int_comp_rec(const uint k, const uint lb, const uint ub,
                        const uint n) {
  uint64_t lRes = 0;
  // recursion end
...
uint64_t f_int_comp_csd_rec(const uint k, const uint lb, const uint ub,
                            const uint n) {
  if ((k * lb) > n) {
    return 0;
  }
...
// a_i in [1,m], n <= m
uint64_t f_int_comp_1_m_geq_n(const uint k, const uint lb, const uint ub,
                              const uint n) {
  assert(1 == lb);
  assert(n <= ub);
...
// a_i in [0,m], n <= m;
uint64_t f_int_comp_0_m_geq_n(const uint k, const uint lb, const uint ub,
                              const uint n) {
  assert(0 == lb);
  assert(n <= ub);
...
uint64_t f_gauss(const uint64_t n) { return ((n * (n + 1)) / 2); }

// number of integer compositions for the special case
// k = 2, a_i in [1,ub]
uint64_t f_int_comp_k_eq_2_lb_eq_1(const uint ub, const uint n) {
...
// summation form
uint64_t f_int_comp_k_eq_3_lb_eq_1_sum(const int ub, const int n) {
  constexpr bool lTrace = false;
  const int m = ub;
  const int lb = 1;
  if constexpr (lTrace) {
    std::cout << "f_int_comp_k_eq_3_lb_eq_1_sum(" << ub << ',' << n
              << "):" << std::endl;
  }
  if (3 > n) {
    return 0;
...
// n' in [d,e], m >= e
inline uint64_t f_int_comp_k_eq_3_case_1(
    const uint64_t m,  // unused for calculations
    const uint64_t d, const uint64_t e, const uint64_t n) {
  return ((e - d + 1) * (2 * m + 1) - f_gauss(e) + f_gauss(d - 1));
}

// n' in [d,e], m < d
inline uint64_t f_int_comp_k_eq_3_case_2(const uint64_t m, const uint64_t d,
                                         const uint64_t e, const uint64_t n) {
  return f_gauss(e) - f_gauss(d - 1) - (e - d + 1);
}

// closed form (see papers/EstCardHaving/intcomp.tex)
uint64_t f_int_comp_k_eq_3_lb_eq_1_csd(const int ub, const int n) {
  constexpr bool lTrace = false;
  const int m = ub;
  const int lb = 1;
  if constexpr (lTrace) {
    std::cout << "f_int_comp_k_eq_3_lb_eq_1_csd(" << ub << ',' << n
              << "):" << std::endl;
  }
  if (3 > n) {
    return 0;
  ...
  if (m < lBegin) {
    // lRes = (lEnd - lBegin + 1) * (2*m + 1) - f_gauss(lEnd) + f_gauss(lBegin -
    // 1);
...
// a_i in [1,m], m <  n;
uint64_t f_int_comp_1_m(const uint k, const uint lb, const uint ub,
                        const uint n) {
  // constexpr bool lTrace = false;
  assert(1 == lb);
  assert(lb < ub);
...
uint64_t f_int_decomp(const uint k, const uint lb, const uint ub,
                      const uint n) {
  return 0;
}
...
uint64_t f_int_comp_0_m_geq_n_leq(const uint k, const uint lb, const uint ub,
                                  const uint n) {
  assert(0 == lb);
  assert(n <= ub);
