...
void loopspec_t::set(const uint aLoA, const uint aHiA, const uint aStepA) {
  _is_eq = true;
  _lo_a = aLoA;
  _hi_a = aHiA;
  _step_a = aStepA;
...
void loopspec_t::generate_queries(genfun_t aGenFun, uint& a, uint& b,
...
void loopspec_t::eval_estimates(checkfun_t aCheckFun, uint& a, uint& b,
  ...
  if (_is_eq) {
    for (uint lValA = _lo_a; lValA <= _hi_a; lValA += _step_a) {
      a = lValA;
      b = lValA;
      (*aCheckFun)(aSimpleProfile, aTpchEst, aQueryInstance, aQueryTemplate,
                   aCb);
    }
  } else {
    uint lValB;
