...
SimpleProfileGbH::SimpleProfileGbH()
    : _card_R(0), _attr_stat_A(), _attr_stat_B(), _attr_stat_Cnt() {}
...
double SimpleProfileGbH::estimate_cnt_eq(const uint aCnt) const {
...
double SimpleProfileGbH::estimate_cnt_between(const uint aLbCnt,
  ...
  return ((lUb - lLb + 1) *
          freq_tot_k(1));  // argument of freq_tot_k ignored anyway
}

double SimpleProfileGbH::estimate_cnt_eq_sel(const uint aCnt,
  ...
  for (uint k = aCnt; k <= max_cnt(); ++k) {
    lRes1 += fBinomGamma(k, aCnt) * std::pow(1 - aSelectivity, k - aCnt) *
             std::pow(aSelectivity, aCnt) * freq_tot_k(aCnt);
  }
  double lRes2 = 0;
  for (uint k = aCnt; k <= max_cnt(); ++k) {
...
double SimpleProfileGbH::estimate_cnt_between_sel(
    const uint aLbCnt, const uint aUbCnt, const double aSelectivity) const {
  const uint lLb = std::max<uint>(min_cnt(), aLbCnt);
...
double SimpleProfileGbH::estimate_sum_eq(const double aSumB) const {
  double lRes = 0;
...
double SimpleProfileGbH::estimate_sum_eq_ic(const uint aSumB) const {
...
double SimpleProfileGbH::estimate_sum_eq_ic_lim(const uint aSumB,
                                                const uint aLim) const {
  const double p = get_p();
  const double F = freq_tot_k_uda();
  double lRes = 0;
  const uint l = min_B();
...
double SimpleProfileGbH::estimate_sum_between(const double aLb,
...
double SimpleProfileGbH::estimate_sum_eq_cnt(const double aSumB,
                                             const uint aLbCnt,
                                             const uint aUbCnt) const {
  double lRes = 0;
...
double SimpleProfileGbH::estimate_sum_between_and_cnt_between(
    const double aLbSum, const double aUbSum, const uint aLbCnt,
    const uint aUbCnt) const {
  double lRes = 0;
  const uint lCntLb = std::max<uint>(min_cnt(), aLbCnt);
  const uint lCntUb = std::min<uint>(max_cnt(), aUbCnt);
...
// TODO
double SimpleProfileGbH::estimate_sum_between_or_cnt_between(
    const double aLbSum, const double aUbSum, const uint aLbCnt,
    const uint aUbCnt) const {
  assert(0 < aLbCnt);
...
double SimpleProfileGbH::estimate_sum_eq_sel(const double aSumB,
                                             const double aSelectivity) const {
  double lRes = 0;
#ifdef USE_SEL_A
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    ...
    for (uint j = 1; j <= k; ++j) {
      ...
      lRes += fBinomGamma(k, j) * std::pow(1 - aSelectivity, k - j) *
              std::pow(aSelectivity, j) * est_sum_eq(j, aSumB);
    }
  }
#else
  double lSumB = aSumB * (1 / aSelectivity);
  if (is_ui_dense_B()) {
    lSumB = (uint)std::max<double>(
        1, std::round((double)aSumB * (1 / aSelectivity)));
  }
  ...
  double lProd =
      (1 - aSelectivity);  // from 1 - (1 - s)^k this contains (1 - s)^k
  if ((min_cnt() <= 1) && (1 <= max_cnt())) {
...
double SimpleProfileGbH::estimate_sum_between_sel(
    const double aLb, const double aUb, const double aSelectivity) const {
  if (aLb > aUb) {
    return 0;
  }
  double lRes = 0;
  ...
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    ...
    for (uint j = 1; j <= k; ++j) {
      // std::cout << "     lRes(2) = " << lRes << std::endl;
      lRes += fBinomGamma(k, j) * std::pow(1 - aSelectivity, k - j) *
              std::pow(aSelectivity, j) * est_sum_between(j, aLb, aUb);
    }
  }
#else
  ...
  if (is_ui_dense_B()) {
    lLb =
        (uint)std::max<double>(1, std::round((double)aLb * (1 / aSelectivity)));
    lUb =
        (uint)std::max<double>(1, std::round((double)aUb * (1 / aSelectivity)));
  }
#ifdef USE_SEL_YAO_APPROX
  double lProd =
      (1 - aSelectivity);  // from 1 - (1 - s)^c this contains (1 - s)^c
  if ((min_cnt() <= 1) && (1 <= max_cnt())) {
...
double SimpleProfileGbH::estimate_avg_between(const double aLbAvgB,
                                              const double aUbAvgB) const {
  double lRes = 0;
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    if (1 == k) {
...
// TODO
double SimpleProfileGbH::estimate_avg_between_and_cnt_between(
    const double aLbAvgB, const double aUbAvgB, const uint aLbCnt,
    const uint aUbCnt) const {
  ...
  for (uint k = lLbCnt; k <= lUbCnt; ++k) {
    lRes +=
        estimate_sum_between_and_cnt_between(k * aLbAvgB, k * aUbAvgB, k, k);
  }
  return lRes;
}

// TODO
double SimpleProfileGbH::estimate_avg_between_or_cnt_between(
    const double aLbAvgB, const double aUbAvgB, const uint aLbCnt,
    const uint aUbCnt) const {
  // const uint lLbCnt = std::max<uint>(aLbCnt, min_cnt());
  // const uint lUbCnt = std::min<uint>(aUbCnt, max_cnt());
  double lRes = 0;
  // for(uint k = lLbCnt; k <= lUbCnt; ++k) {
  //   lRes += estimate_sum_between_and_cnt_between(k * aLbAvgB, k * aUbAvgB, k,
  //   k);
  // }
  return lRes;
}
...
// TODO
double SimpleProfileGbH::estimate_avg_between_sel(
    const double aLb, const double aUb, const double aSelectivity) const {
  double lRes = 0;
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    for (uint j = 1; j <= k; ++j) {
      lRes += fBinomGamma(k, j) * std::pow(1 - aSelectivity, k - j) *
              std::pow(aSelectivity, j) * est_sum_between(j, j * aLb, j * aUb);
    }
  }
  return lRes;
}
...
double SimpleProfileGbH::estimate_min_eq(const double aVal) const {
  ...
  double lRes = 0;
  const double b =
      (aVal - min_B()) / (max_B() - min_B());  // prob one in [min_b,aVal)
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    if (1 == k) {
      lRes += get_p() * freq_tot_k(1);
    } else {
      lRes += k * get_p() * std::pow(1 - b, k - 1) *
              freq_tot_k(k);  // clearly too high
    }
  }
#endif
#ifdef ALT_MINMAX_B
  ...
  if (aVal < (max_B() - avg_dist_B())) {
    for (uint k = min_cnt(); k <= max_cnt(); ++k) {
      ...
      if (max_B() < (aVal + k * avg_dist_B())) {
        break;
      }
      lFak = mt::fLogBinomGamma<double>(
                 ((max_B() - aVal + avg_dist_B()) / avg_dist_B()), k - 1) -
...
double SimpleProfileGbH::estimate_min_eq_cnt(const double aVal,
                                             const uint aLbCnt,
                                             const uint aUbCnt) const {
  if ((aVal < min_B()) || (max_B() < aVal)) {
    return 0;
...
double SimpleProfileGbH::estimate_min_eq_sel(const double aVal,
                                             const double aSelectivity) const {
  double lRes = 0;
  const double b =
      (aVal - min_B()) / (max_B() - min_B());  // prob one in [min_b,aVal)
  ...
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    ...
    for (uint j = 1; j <= k; ++j) {
      ...
      lRes += fBinomGamma(k, j) * std::pow(1 - aSelectivity, k - j) *
              std::pow(aSelectivity, j) * lEstMinEq;
    }
  }
  return lRes;
}

double SimpleProfileGbH::estimate_min_between(const double aLb,
                                              const double aUb) const {
  ...
  double lRes = 0;
  const double a =
      (lUb - lLb + avg_dist_B()) / nodv_B();  // prob one in [lb,ub]
  const double b =
      (lLb - min_B() + avg_dist_B()) / nodv_B();  // prob one in [min,lb]
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    if (1 == k) {
      lRes += a * freq_tot_k(1);
    } else {
...
double SimpleProfileGbH::estimate_min_between_and_cnt_between(
    const double aLbMinB, const double aUbMinB, const uint aLbCnt,
    const uint aUbCnt) const {
...
double SimpleProfileGbH::estimate_min_between_sel(
    const double aLbMinB, const double aUbMinB,
    const double aSelectivity) const {
  const double lLb = std::max(aLbMinB, min_B());
  const double lUb = std::min(aUbMinB, max_B());
  if (lUb < lLb) {
  ...
  double lRes = 0;
  const double a = ((lUb - lLb + avg_dist_B()) / avg_dist_B()) /
                   nodv_B();  // prob one in [lb,ub]
  const double b = ((lLb - min_B() + avg_dist_B()) / avg_dist_B()) /
                   nodv_B();  // prob one in [min,lb]
  double lEstMinRg = 0;
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    for (uint j = 1; j <= k; ++j) {
      if (1 == j) {
      ...
      lRes += fBinomGamma(k, j) * std::pow(1 - aSelectivity, k - j) *
              std::pow(aSelectivity, j) * lEstMinRg;
    }
  }
  return lRes;
}
...
double SimpleProfileGbH::estimate_max_eq(const double aVal) const {
  if ((aVal < min_B()) || (max_B() < aVal)) {
    return 0;
  }
#ifdef ALT_MINMAX_A
  double lRes = 0;
  const double a =
      (aVal - min_B()) / (max_B() - min_B());  // prob one in [min_b, b]
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    if (1 == k) {
      lRes += get_p() * freq_tot_k(1);
    } else {
      lRes +=
          k * get_p() * std::pow(a, k - 1) * freq_tot_k(k);  // clearly too high
    }
  }
#endif
#ifdef ALT_MINMAX_B
  ...
  if (aVal > (min_B() + avg_dist_B())) {
    for (uint k = min_cnt(); k <= max_cnt(); ++k) {
      ...
      lFak = mt::fLogBinomGamma<double>(
                 ((aVal - min_B() + avg_dist_B()) / avg_dist_B()), k - 1) -
             mt::fLogBinomGamma<double>(nodv_B(), k);
      // std::cout << "k = " << k
      //           << ", fak = " << std::exp(lFak)
      //           << ", inc = " << std::exp(lFak) * freq_tot_k(k)
...
double SimpleProfileGbH::estimate_max_eq_cnt(const double aVal,
                                             const uint aLbCnt,
                                             const uint aUbCnt) const {
  if ((aVal < min_B()) || (max_B() < aVal)) {
    return 0;
...
double SimpleProfileGbH::estimate_max_eq_sel(const double aVal,
                                             const double aSelectivity) const {
  double lRes = 0;
  const double a =
      (aVal - min_B()) / (max_B() - min_B());  // prob one in [min_b, b]
  ...
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    for (uint j = 1; j <= k; ++j) {
      ...
      lRes += fBinomGamma(k, j) * std::pow(1 - aSelectivity, k - j) *
              std::pow(aSelectivity, j) * lEstMaxEq;
    }
  }
  return lRes;
}

double SimpleProfileGbH::estimate_max_between(const double aLbMaxB,
                                              const double aUbMaxB) const {
  ...
  const double a = ((lUb - lLb + avg_dist_B()) / avg_dist_B()) /
                   nodv_B();  // prob one in [lb,ub]
  const double b = ((max_B() - lUb + avg_dist_B()) / avg_dist_B()) /
                   nodv_B();  // prob one in [ub,max]
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    if (1 == k) {
      lRes += a * freq_tot_k(1);
    } else {
...
double SimpleProfileGbH::estimate_max_between_and_cnt_between(
    const double aLbMaxB, const double aUbMaxB, const uint aLbCnt,
    const uint aUbCnt) const {
  const double lLb = std::max(aLbMaxB, min_B());
  const double lUb = std::min(aUbMaxB, max_B());
  if (lUb < lLb) {
  ...
  const double a =
      (lUb - lLb + avg_dist_B()) / nodv_B();  // prob one in [lb,ub]
  const double b =
      (max_B() - lUb + avg_dist_B()) / nodv_B();  // prob one in [ub,max]
  for (uint k = lLbCnt; k <= lUbCnt; ++k) {
    if (1 == k) {
      lRes += a * freq_tot_k(1);
    } else {
...
double SimpleProfileGbH::estimate_max_between_sel(
    const double aLbMaxB, const double aUbMaxB,
    const double aSelectivity) const {
  const double lLb = std::max(aLbMaxB, min_B());
  const double lUb = std::min(aUbMaxB, max_B());
  if (lUb < lLb) {
  ...
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    for (uint j = 1; j <= k; ++j) {
      ...
      lRes += fBinomGamma(k, j) * std::pow(1 - aSelectivity, k - j) *
              std::pow(aSelectivity, j) * lEstMaxRg;
    }
  }
  return lRes;
}
...
double SimpleProfileGbH::est_sum_eq(const uint aK, const double aVal) const {
  if (1 == aK) {
    return est_sum_1_eq(aVal);
  }
  if (2 == aK) {
    return est_sum_2_eq(aVal);
  }
  if (3 == aK) {
    return est_sum_3_eq(aVal);
  }
  return est_sum_k_eq(aK, aVal);
}

double SimpleProfileGbH::est_sum_1_eq(const double aVal) const {
...
double SimpleProfileGbH::est_sum_2_eq(const double aVal) const {
  if ((min_B_k(2) > aVal) || (max_B_k(2) < aVal)) {
    return 0;
  }
  // std::cout << std::endl;
  // std::cout << "no_comp_2(2, aVal, 50) = " << no_comp_2(2, aVal, 50) <<
  // std::endl;
...
double SimpleProfileGbH::est_sum_3_eq(const double aVal) const {
  if ((min_B_k(2) > aVal) || (max_B_k(2) < aVal)) {
    return 0;
  }
  // std::cout << std::endl;
  // std::cout << "no_comp_2(3, aVal, 50) = " << no_comp_2(3, aVal, 50) <<
  // std::endl;
...
double SimpleProfileGbH::est_sum_k_eq(const uint aK, const double aVal) const {
...
double SimpleProfileGbH::est_sum_between(const uint aK, const double aLb,
                                         const double aUb) const {
  if (1 == aK) {
    return est_sum_1_between(aLb, aUb);
  }
  if (2 == aK) {
    return est_sum_2_between(aLb, aUb);
  }
  return est_sum_k_between(aK, aLb, aUb);
}

double SimpleProfileGbH::est_sum_1_between(const double aLb,
                                           const double aUb) const {
  double a = aLb;
  double b = aUb;
  cut_B_k(1, a, b);
...
double SimpleProfileGbH::est_sum_2_between(const double aLb,
                                           const double aUb) const {
  double a = aLb;
  double b = aUb;
  cut_B_k(2, a, b);
...
double SimpleProfileGbH::est_sum_k_between(const uint aK, const double aLb,
                                           const double aUb) const {
  double a = aLb;
  double b = aUb;
  cut_B_k(aK, a, b);
...
void SimpleProfileGbH::cut_B_k(const uint k, double& a, double& b) const {
  a = std::max<double>(a, min_B_k(k));
  b = std::min<double>(b, max_B_k(k));
};

void SimpleProfileGbH::cut_cnt(uint& a, uint& b) const {
...
// or by Yao
double SimpleProfileGbH::sp(const uint k, const double aSelectivity) const {
  double lRes = 0;
#ifdef USE_SEL_YAO_APPROX
...
std::ostream& SimpleProfileGbH::print(std::ostream& os) const {
  ...
  os << "  |R|        = " << std::setw(8) << card_R() << std::endl
     ...
     << "  is_ui_dns  = " << std::setw(8)
     << (is_ui_dense_B() ? "true" : "false") << std::endl
     << std::endl;
  os << "     k    min_k    max_k   freq_k   mean_k stddev_k    var_k"
     << std::endl;
  for (uint k = min_cnt(); k <= max_cnt(); ++k) {
    os << "    " << std::setw(2) << k << ' ' << std::setw(8) << min_B_k(k)
       << ' ' << std::setw(8) << max_B_k(k) << ' ' << std::setw(8)
       << freq_tot_k(k) << ' ' << std::setw(8) << mean_B_k(k) << ' '
       << std::setw(8) << std_dev_B_k(k) << ' ' << std::setw(8)
       << variance_B_k(k) << std::endl;
  }
  os << "----" << std::endl;
  os << std::endl;
  return os;
