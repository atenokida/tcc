...
EstHavingSqlServer::dist_t EstHavingSqlServer::get_dist_Cnt() {
...
EstHavingSqlServer::dist_t EstHavingSqlServer::get_dist_B() {
  return dist_t(min_B(), max_B(), mean_B(), std_dev_B());
}

double EstHavingSqlServer::estimate_cnt_eq(const uint aCnt) const {
  return ((double)nodv_A() * selectivity_cnt_eq(aCnt));
}

double EstHavingSqlServer::estimate_cnt_between(const uint aLbCnt,
                                                const uint aUbCnt) const {
  return ((double)nodv_A() * selectivity_cnt_between(aLbCnt, aUbCnt));
}

double EstHavingSqlServer::selectivity_cnt_eq(const uint aCnt) const {
...
double EstHavingSqlServer::selectivity_cnt_between(const uint aLbCnt,
                                                   const uint aUbCnt) const {
  double l = 0, u = 0;
  if (1 >= aLbCnt) {
    l = _dist_Cnt.get_cdf(1.0);
