...
TpchEst::TpchEst()
  ...
  if (lTrace) {
    ...
    for (uint k = 1; k <= 7; ++k) {
      std::cout << std::setw(3) << k << "  " << std::setw(8) << _freq_tot[k]
                << std::endl;
    }
    std::cout << std::endl;
  }
  ...
  // calculate mean and standard deviation (goal: approximate by normal
  // distribution)
  if (lTrace) {
  ...
  for (uint k = 1; k <= 7; ++k) {
    ...
    lVar.fin();
    if (lTrace) {
      std::cout << std::setw(3) << k << ' ' << std::setw(8) << lVar.mean()
                << ' ' << std::setw(8) << lVar.variance() << ' ' << std::setw(8)
                << lVar.standardDeviation() << std::endl;
    }
    _mean[k] = lVar.mean();
...
#ifdef USE_BOOST
double TpchEst::pdf_k(const uint k, const double a) const {
  dist_t lDist(mean(k), std_dev(k));
  return pdf(lDist, a);
}

double TpchEst::cdf_k(const uint k, const double a) const {
  dist_t lDist(mean(k), std_dev(k));
  return cdf(lDist, a);
}
#else
...
std::ostream& TpchEst::print_a(std::ostream& os) const {
  for (uint c = 0; c <= 350; ++c) {
    os << std::setw(3) << c;
    for (uint k = 1; k <= 7; ++k) {
...
double TpchEst::estimate_sum_eq(const uint aSumQuantity) const {
  double lRes = 0;
...
double TpchEst::estimate_sum_between(const uint aSumQuantityLb,
                                     const uint aSumQuantityUb) const {
  double lRes = 0;
...
double TpchEst::estimate_sum_eq_cnt(const uint aSumQuantity, const uint aCntLb,
                                    const uint aCntUb) const {
  double lRes = 0;
  ...
  for (uint k = std::max<uint>(3, aCntLb); k <= std::min<uint>(7, aCntUb);
       ++k) {
    lRes += est_k_eq(k, aSumQuantity);
  }
  return lRes;
...
double TpchEst::estimate_sum_between_and_cnt_between(const uint aLb,
                                                     const uint aUb,
                                                     const uint aCntLb,
                                                     const uint aCntUb) const {
  double lRes = 0;
...
double TpchEst::estimate_sum_eq_sel(const uint aSumQuantity,
                                    const double aSelectivity) const {
  double lRes = 0;
  uint lSumQuantity = (uint)std::max<double>(
      1, std::round((double)aSumQuantity * (1 / aSelectivity)));
  double lProd =
...
double TpchEst::estimate_sum_between_sel(const uint aLb, const uint aUb,
                                         const double aSelectivity) const {
  double lRes = 0;
  uint lLb =
...
double TpchEst::est_1_eq(const uint aVal) const {
...
double TpchEst::est_2_eq(const uint aVal) const {
...
double TpchEst::est_k_eq(const uint aK, const uint aVal) const {
...
double TpchEst::est_1_between(const uint aLb, const uint aUb) const {
  uint a = aLb;
  uint b = aUb;
  cut_k(1, a, b);
  if (a > b) {
...
double TpchEst::est_2_between(const uint aLb, const uint aUb) const {
  uint a = aLb;
  uint b = aUb;
  cut_k(2, a, b);
  double lRes = 0;
...
double TpchEst::est_k_between(const uint aK, const uint aLb,
                              const uint aUb) const {
  uint a = aLb;
  uint b = aUb;
  cut_k(aK, a, b);
  if (b < a) {
...
void TpchEst::cut_k(const uint k, uint& a, uint& b) const {
  a = std::max<uint>(a, min_B_k(k));
  b = std::min<uint>(b, max_B_k(k));
};

uint TpchEst::min_B_k(const uint aK) const { return aK; };

uint TpchEst::max_B_k(const uint aK) const { return (aK * 51); };

uint TpchEst::get_true_card_cnt_eq(const uint aCnt) const {
  if ((0 == aCnt) || (7 < aCnt)) {
    return 0;
  }
...
uint TpchEst::get_true_card_cnt_between(const uint aLbCnt,
...
uint TpchEst::get_true_card_sum_eq(const uint aVal) const {
  uint lRes = 0;
  for (uint k = 1; k <= 7; ++k) {
...
uint TpchEst::get_true_card_sum_eq_and_cnt_between(const uint aVal,
                                                   const uint aCntLb,
                                                   const uint aCntUb) const {
  uint lRes = 0;
  for (uint k = std::max<uint>(1, aCntLb); k <= std::min<uint>(7, aCntUb);
       ++k) {
    lRes += freq_sum(k, aVal);
  }
  return lRes;
...
uint TpchEst::get_true_card_sum_between(const uint aLb, const uint aUb) const {
  uint lRes = 0;
  for (uint k = 1; k <= 7; ++k) {
...
uint TpchEst::get_true_card_sum_between_and_cnt_between(
    const uint aLb, const uint aUb, const uint aLbCnt,
    const uint aUbCnt) const {
  uint lRes = 0;
  for (uint k = std::max<uint>(1, aLbCnt); k <= std::min<uint>(7, aUbCnt);
       ++k) {
...
uint TpchEst::get_true_card_sum_between_or_cnt_between(
    const uint aLb, const uint aUb, const uint aLbCnt,
    const uint aUbCnt) const {
  uint lRes = 0;
  for (uint k = 1; k <= 7; ++k) {
...
uint TpchEst::get_true_card_avg_between(const uint aLb, const uint aUb) const {
  uint lRes = 0;
  for (uint k = 1; k <= 7; ++k) {
...
uint TpchEst::get_true_card_avg_between_cnt(const uint aLb, const uint aUb,
                                            const uint aLbCnt,
                                            const uint aUbCnt) const {
  uint lRes = 0;
  for (uint k = std::max<uint>(1, aLbCnt); k <= std::min<uint>(7, aUbCnt);
...
uint TpchEst::get_true_card_avg_between_one_count(const uint aLb,
                                                  const uint aUb,
                                                  const uint aCnt) const {
