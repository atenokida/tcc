...
EstHavingFentSn::dist_t EstHavingFentSn::get_dist_Cnt() {
...
EstHavingFentSn::dist_t EstHavingFentSn::get_dist_B() {
  return dist_t(min_B(), max_B(), dist_param_B());
}

EstHavingFentSn::dist_t EstHavingFentSn::get_dist_sum() {
  _dist_param_sum = dist_param_B() * _dist_param_Cnt;
  return dist_t(1 * min_B(),
                (((double)card_R()) / ((double)nodv_A())) * max_B(),
                _dist_param_sum);
}

double EstHavingFentSn::estimate_cnt_eq(const uint aCnt) const {
...
double EstHavingFentSn::estimate_cnt_between(const uint aLbCnt,
...
double EstHavingFentSn::estimate_cnt_eq_sel(const uint aCnt,
                                            const double aSelectivity) const {
  return 1;
}

double EstHavingFentSn::estimate_cnt_between_sel(
    const uint aLbCnt, const uint aUbCnt, const double aSelectivity) const {
  return 1;
}

double EstHavingFentSn::estimate_sum_eq(const double aSumB) const {
  double l = 0, u = 0;
...
double EstHavingFentSn::estimate_sum_eq_sel(const double aSumB,
                                            const double aSelectivity) const {
  return 1;
}

double EstHavingFentSn::estimate_sum_eq_cnt(const double aSumB,
                                            const uint aCntLb,
                                            const uint aCntUb) const {
  return 1;
}

double EstHavingFentSn::estimate_sum_between(const double aLbSumB,
...
double EstHavingFentSn::estimate_sum_between_sel(
    const double aLbSumB, const double aUbSumB,
    const double aSelectivity) const {
  return 1;
}

double EstHavingFentSn::estimate_sum_between_and_cnt_between(
