...
struct Hash_table {
  ...
  Hash_table(int bucket_sum, int _hash_seed = 1000) {
    n = bucket_sum;
    Id = new unsigned *[n]();
    for (int i = 0; i < n; i++) Id[i] = new unsigned[m]();
    Key = new unsigned *[n]();
    for (int i = 0; i < n; i++) Key[i] = new unsigned[m]();
    for (int i = 0; i < n; i++)
      for (int j = 0; j < m; j++) Id[i][j] = Key[i][j] = 0;
  }
...
class Classifier : public Sketch {
 ...
 public:
  Classifier(int _w, int _d, int _hash_seed = 1000) {
    ...
    midsize = n * m;
    Mid = new unsigned *[n];
    for (int i = 0; i < n; i++) Mid[i] = new unsigned[m]();
    _w -= midsize * 4;
...
#define Mid_Key(x) (x & 1023)
  ...
  long double Join(Sketch *_other) {
    ...
    for (int i = 0; i < other->Heavy->n; i++)
      for (int j = 0; j < other->Heavy->m; j++)
        if (other->Heavy->solid(i, j)) {
          re +=
              1.0 * QueryMid(other->Heavy->Id[i][j]) * other->Heavy->Key[i][j];
        }
    for (int i = 0; i < n; i++)
      for (int j = 0; j < m; j++)
        if (Mid[i][j] != 0) {
    ...
    for (int i = 0; i < n; i++)
      for (int j = 0; j < m; j++)
        if (other->Mid[i][j] != 0) {
          re += 1.0 * Mid_Key(other->Mid[i][j]) *
                Light->Query(Mid_Id(other->Mid[i][j]));
        }
    return re;
  }
  ...
  ~Classifier() {
    for (int i = 0; i < n; i++) delete[] Mid[i];
    delete[] Mid;
    Heavy->~Hash_table();
