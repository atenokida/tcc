...
int cms_init_optimal_alt(CountMinSketch* cms, double error_rate,
  ...
  if (error_rate < 0 || confidence < 0) {
    fprintf(stderr,
            "Unable to initialize the count-min sketch since both error_rate "
            "and confidence must be positive!\n");
    return CMS_ERROR;
  }
...
int cms_init_alt(CountMinSketch* cms, uint32_t width, uint32_t depth,
  ...
  if (depth < 1 || width < 1) {
    fprintf(stderr,
            "Unable to initialize the count-min sketch since either width or "
            "depth is 0!\n");
    return CMS_ERROR;
  }
...
int32_t cms_add_inc_alt(CountMinSketch* cms, uint64_t* hashes,
  ...
  if (num_hashes < cms->depth) {
    fprintf(stderr,
            "Insufficient hashes to complete the addition of the element to "
...
int32_t cms_remove_inc_alt(CountMinSketch* cms, uint64_t* hashes,
  ...
  if (num_hashes < cms->depth) {
    fprintf(stderr,
            "Insufficient hashes to complete the removal of the element to the "
            "count-min sketch!");
    return CMS_ERROR;
  }
...
int32_t cms_check_alt(CountMinSketch* cms, uint64_t* hashes,
  ...
  if (num_hashes < cms->depth) {
    fprintf(stderr,
            "Insufficient hashes to complete the min lookup of the element to "
            "the count-min sketch!");
    return CMS_ERROR;
  }
  int32_t num_add = INT32_MAX;
...
int32_t cms_check_mean_alt(CountMinSketch* cms, uint64_t* hashes,
  ...
  if (num_hashes < cms->depth) {
    fprintf(stderr,
            "Insufficient hashes to complete the mean lookup of the element to "
            "the count-min sketch!");
    return CMS_ERROR;
  }
  int32_t num_add = 0;
...
int32_t cms_check_mean_min_alt(CountMinSketch* cms, uint64_t* hashes,
  ...
  if (num_hashes < cms->depth) {
    fprintf(stderr,
            "Insufficient hashes to complete the mean-min lookup of the "
...
int cms_merge(CountMinSketch* cms, int num_sketches, ...) {
  ...
  if (CMS_ERROR == res) return CMS_ERROR;
...
** its applications" by Cormode & Muthukrishnan.
*------------------------------------------------------------------------**/
// Wrapper function for different types of predicates.
    ...
    const bool col_homogeneous)
{
  ...
  if (col_homogeneous) {
    ...
    // So we can just calculate the inner product of each row with itself.
    // The solution is the same for complex predicate since we concatenate
    // the attributes appearing in the predicates.
    float min = std::numeric_limits<float>::max();
    for (std::size_t i = 0; i < cms.depth; ++i) {
      float row_inner_prod = 0;
      uint32_t bin = i * cms.width;
      for (std::size_t j = 0; j < cms.width; ++j) {
        row_inner_prod += cms.bins[bin + j] * cms.bins[bin + j];
      }
